...
public class S3AwsService(IAmazonS3 client, IConfiguration configuration, ILogger<S3AwsService> logger) : IS3Service
{
    ...
    public async Task<bool> UploadFile(string fileData)
    {
        ...
        logger.LogInformation("Began uploading project {file} onto {bucket}", id, _bucketName);
        var request = new PutObjectRequest
        {
            BucketName = _bucketName,
            Key = $"project_{id}.json",
            InputStream = stream
        };
        ...
        {
            logger.LogError("Failed to upload project {file}: {code}", id, response.HttpStatusCode);
            return false;
        }
        logger.LogInformation("Finished uploading project {file} to {bucket}", id, _bucketName);
        return true;
    }
